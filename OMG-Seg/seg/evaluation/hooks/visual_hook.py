...
class SAMLocalVisualizer(DetLocalVisualizer):
    ...
    def save_seg(
        ...
        if 'pred_panoptic_seg' in data_sample:
            ...
            #     classes, palette)
            pred_img_data = self._save_panoptic_seg(
                pred_img_data, data_sample.pred_panoptic_seg.numpy(),
                classes, palette)

        if out_file is not None:
            np.save(out_file, pred_img_data.astype(np.uint16))
            
    def _save_panoptic_seg(self, image: np.ndarray,
        ...
        check_type('binary_masks', segms, (np.ndarray, torch.Tensor))
        binary_masks = tensor2ndarray(segms)
        ...
        instance_idx = 1
        sem_map = np.zeros((binary_masks.shape[1], binary_masks.shape[2], 2))
        for binary_mask, label in zip(binary_masks, labels):

            instance_mask = binary_mask * instance_idx
            label_mask = binary_mask * (label + 1)

            sem_map[..., 0] = sem_map[..., 0] + instance_mask
            sem_map[..., 1] = sem_map[..., 1] + label_mask
