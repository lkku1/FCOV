...
def config_eval_dataloader(args):
    ...
    flow_gaps = [int(i) for i in args.flow_gaps.split(",")]
    reverse = [int(i) for i in args.reverse.split(",")]
    for reverse_idx in reverse:
        for gap_idx in flow_gaps:
            flow_gaps_list.append(int(reverse_idx * gap_idx))

    ref_sam = sam_model_registry[args.rgb_encoder](checkpoint= os.path.join(args.check_dir, args.rgb_encoder_ckpt_path))

    val_data_dirs = [os.path.join(args.save_dir, args.name + "_middle_flow"), os.path.join(args.save_dir, args.name + "_middle_rgb")]
    val_seq = None 
    val_dataset = Example_eval_dataset(data_dirs=val_data_dirs, seqs=val_seq, ref_sam=ref_sam, flow_gaps=flow_gaps_list, num_gridside=args.num_gridside)
    val_loader = DataLoader(val_dataset, num_workers=0, batch_size=1, shuffle=False, pin_memory=True, drop_last=False)
