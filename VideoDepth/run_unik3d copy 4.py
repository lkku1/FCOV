...
def vis():
    ...
    # image = torch.tensor(image)[None].permute(0, 3, 1, 2).float()
    # image = equi2pers(image.float(), 1, (depth_size, depth_size), mode="bilinear")[0, :, :, :, 0].permute(1, 2, 0).numpy().astype(np.uint8)
    depth = read_exr_file("D:/linux/github2/test_image/preprocess/cafeteria_init_depth/00000.exr")


    # image1 = cv2.imread("D:/linux/github2/test_image/image/cafeteria/background.png")
    # image1 = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    # image1 = cv2.resize(image, (432, 240))

    # depth1 = read_exr_file("D:/linux/github2/test_image/preprocess/cafeteria_depth/background.exr")
    # depth1 = cv2.resize(depth1, (432, 240))
    # height, _ = torch.meshgrid(torch.linspace(-0.5+1e-6, 0.5-1e-6, 240), torch.linspace(1e-6, 1.0-1e-6, 432), indexing="ij")
    
    # depth = (1/depth - 1/depth.max())/(1/depth.min() - 1/depth.max()) * 255
    # cv2.imwrite("1.png", depth.astype(np.uint8))
    # depth = np.load("D:/linux/github2/test_image/preprocess/cafeteria_per_depth/per_depth_0.npz")["arr_0"]
    # depth = depth[100]
    # depth = depth / np.cos(height.numpy() * 3.1415926)
    # a = (1/depth - 1/depth.max())/(1/depth.min() - 1/depth.max())*255
    # cv2.imwrite("1.png", a.astype(np.uint8))
    # depth = cv2.medianBlur(depth, 3)
    # depth = depth[:, add_length:-add_length]
    # cv2.imwrite("1.png", image)
    # cv2.imwrite("2.png", depth.astype(np.uint8))

    # image2 = cv2.imread("D:/linux/github2/test_image/image/cafeteria/00007.png")
    
    # image = torch.tensor(image)[None].permute(0, 3, 1, 2).float()
    # image = equi2pers(image.float(), 1, (depth_size, depth_size), mode="bilinear")[0, :, :, :, 3].permute(1, 2, 0).numpy().astype(np.uint8)
    # depth2 = read_exr_file("D:/linux/github2/test_image/preprocess/cafeteria_init_depth/00007.exr")
...
if __name__ == "__main__":
    ...
    # a = read_exr_file("D:/linux/github2/360_train/480p_condition/3bb0500a-5b47-420f-9730-2a9000611983_9_depth/00000.exr")
    # b = read_exr_file("D:/linux/github2/360_train/480p_condition/3bb0500a-5b47-420f-9730-2a9000611983_9_depth/00004.exr")

    # c = read_exr_file("D:/linux/github2/360_train/480p_condition/3bb0500a-5b47-420f-9730-2a9000611983_9_init_depth/00000.exr")
    # d = read_exr_file("D:/linux/github2/360_train/480p_condition/3bb0500a-5b47-420f-9730-2a9000611983_9_init_depth/00004.exr")
    # f = np.abs(b - a)
    # g = np.abs(d - c)
    ...
    # cv2.imwrite("1.png", f.astype(np.uint8))
    # cv2.imwrite("2.png", g.astype(np.uint8))
