...
def depth_estimation(omni_rgb_ori, sky_sem, omni_camera):
...
def run(dataset, option):
    ...
    background = cv2.imread(os.path.join(option.data_dir, name + ".png"))
    background = cv2.cvtColor(background, cv2.COLOR_BGR2RGB)
    sem_dir = option.output_dir[:-5] + "sem"
    background_sem = cv2.imread(os.path.join(sem_dir, name + ".png"), -1)
    background_sky = np.where(background_sem == 2, 1, 0)

    background_depth = depth_estimation(background, background_sky, omni_camera)
...
if __name__ == "__main__":
    ...
    dataset_ = ImageDataset(option_.data_dir, option_.output_dir, option_.width, option_.height)
