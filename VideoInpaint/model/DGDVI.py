...
from model.DeformConv2d_sphe import DeformConv2d_sphe
import os
...
class DepthCompletion(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        for index in range(stack_num):
            ...
            if index % 2== 0:
                load_lora = True
            blocks.append(TransformerBlock(hidden=hidden_d, dim_hidden=fusion_dim_d, num_head=num_head, dropout=dropout, n_vecs=n_vecs, t2t_params=t2t_params, load_lora=load_lora))
        ...
        self.decoder_depth = nn.Sequential(
            ...
            # nn.ReLU(True)
            deconv(channel_d, 32, kernel_size=3, padding=1, deform=True),
            nn.LeakyReLU(0.2, inplace=True),
            deconv(32, 16, kernel_size=3, padding=1 , deform=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(16, 1, kernel_size=1, stride=1, padding=0),
            nn.ReLU(True)
        )
...
class ContentReconstruction(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        for i, d in enumerate(kernel_size):
            n_vecs *= int((output_size[i] + 2 * padding[i] - (d - 1) - 1) / stride[i] + 1)
        for index in range(stack_num[0]):
            load_lora = False
            if index % 2== 0:
                load_lora = True
            blocks1.append(TransformerBlockWithDepth(hidden1=hidden_c, hidden2=hidden_d, dim_hidden1=fusion_dim_c, dim_hidden2=fusion_dim_d, num_head=num_head, dropout=dropout, n_vecs=n_vecs, t2t_params=t2t_params,load_lora=load_lora))
        self.transformer1 = nn.Sequential(*blocks1)
        for index in range(stack_num[1]):
            load_lora = False
            if index % 2== 0:
                load_lora = True
            blocks2.append(TransformerBlock(hidden=hidden_c, dim_hidden=fusion_dim_c, num_head=num_head, dropout=dropout, n_vecs=n_vecs, t2t_params=t2t_params, load_lora=load_lora))
        ...
        self.decoder = nn.Sequential(
            ...
            deconv(channel_c, 128, kernel_size=3, padding=1, deform=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            deconv(64, 64, kernel_size=3, padding=1, deform=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1)
        )
...
class ContentEnhancement(BaseNetwork):
    def __init__(self,
        ...
        self.decoder = nn.Sequential(
            ...
            # nn.LeakyReLU(0.2, inplace=True),
            # nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1)
            deconv(dim, 128, kernel_size=3, padding=1, deform=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            deconv(64, 64, kernel_size=3, padding=1, deform=False),
...
class Encoder(nn.Module):
    def __init__(self, dim):
        ...
        self.layers = nn.ModuleList([
            ...
            DeformConv2d_sphe(3, 64, kernel_size=3, stride=2, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
...
class DepthEncoder(nn.Module):
    def __init__(self, dim):
        ...
        self.layers = nn.Sequential(
            DeformConv2d_sphe(2, 64, kernel_size=3, stride=2, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),
            ...
            # DeformConv2d_sphe(2, 64, kernel_size=3, stride=2, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(64, 64, kernel_size=3, stride=1, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(64, 128, kernel_size=3, stride=2, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(128, 128, kernel_size=3, stride=1, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(128, dim , kernel_size=3, stride=1, padding=1),
            # nn.LeakyReLU(0.2, inplace=True)
        )
...
class DepthEncoder_light(nn.Module):
    def __init__(self, dim):
        ...
        self.layers = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True)
            # DeformConv2d_sphe(1, 32, kernel_size=3, stride=2, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(32, 32, kernel_size=3, stride=1, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(32, 64, kernel_size=3, stride=2, padding=1),
            # nn.LeakyReLU(0.2, inplace=True),
            # DeformConv2d_sphe(64, dim , kernel_size=3, stride=1, padding=1),
            # nn.LeakyReLU(0.2, inplace=True)
        )
...
class deconv(nn.Module):
    def __init__(self, input_channel, output_channel, kernel_size=3, padding=0, deform=True):
        super().__init__()

        # self.conv = nn.Conv2d(input_channel, output_channel,
        #                       kernel_size=kernel_size, stride=1, padding=padding)
        if deform:
            self.conv = DeformConv2d_sphe(input_channel, output_channel, kernel_size=kernel_size, stride=1, padding=padding)
        else:
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, d_model, head, p=0.1, load_lora=False):
        super().__init__()
        if load_lora:
            self.query_embedding = lora.Linear(d_model, d_model, r=8)
            self.value_embedding = lora.Linear(d_model, d_model, r=8)
        else:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, hidden=512, dim_hidden=40, num_head=4, dropout=0.1, n_vecs=None, t2t_params=None, load_lora=False):
        super().__init__()
        self.attention = MultiHeadedAttention(d_model=hidden, head=num_head, p=dropout, load_lora=load_lora)
...
class MultiHeadedAttentionWithDepth(nn.Module):
    ...
    def __init__(self, d1_model, d2_model, head, p=0.1, load_lora=False):
        super().__init__()
        if load_lora:
            self.query_embedding = lora.Linear(d1_model+d2_model, d1_model+d2_model)
            self.color_value_embedding = lora.Linear(d1_model, d1_model)
            self.depth_value_embedding = lora.Linear(d2_model, d2_model)
        else:
...
class TransformerBlockWithDepth(nn.Module):
    ...
    def __init__(self, hidden1=512, hidden2=128, dim_hidden1=40, dim_hidden2=16, num_head=4, dropout=0.1, n_vecs=None, t2t_params=None, load_lora=False):
        super().__init__()
        self.attention = MultiHeadedAttentionWithDepth(d1_model=hidden1, d2_model=hidden2, head=num_head, p=dropout, load_lora=load_lora)
...
class DCNv2PackFlowGuided(ModulatedDeformConvPack):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.index == 0:
            self.conv_offset = nn.Sequential(
                DeformConv2d_sphe((1+self.pa_frames//2) * self.in_channels + self.pa_frames, self.out_channels, 3, 1, 1),
                nn.LeakyReLU(negative_slope=0.1, inplace=True),
                nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1),
                nn.LeakyReLU(negative_slope=0.1, inplace=True),
                nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1),
                nn.LeakyReLU(negative_slope=0.1, inplace=True),
                lora.Conv2d(self.out_channels, 3 * 9 * self.deformable_groups, 3, 16, 1, padding=1),
            )
        else:
            self.conv_offset = nn.Sequential(
    ...
    def init_offset(self):
        super(ModulatedDeformConvPack, self).init_weights()
        if hasattr(self, 'conv_offset'):
            self.conv_offset[-1].conv.weight.data.zero_()
            self.conv_offset[-1].conv.bias.data.zero_()
...
class BasicModule(nn.Module):
    ...
    def __init__(self):
        ...
        self.basic_module = nn.Sequential(
            # DeformConv2d_sphe(in_channels=8, out_channels=32, kernel_size=7, stride=1, padding=3), nn.ReLU(inplace=False),
            # DeformConv2d_sphe(in_channels=32, out_channels=64, kernel_size=7, stride=1, padding=3), nn.ReLU(inplace=False),
            # DeformConv2d_sphe(in_channels=64, out_channels=32, kernel_size=7, stride=1, padding=3), nn.ReLU(inplace=False),
            # DeformConv2d_sphe(in_channels=32, out_channels=16, kernel_size=7, stride=1, padding=3), nn.ReLU(inplace=False),
            # DeformConv2d_sphe(in_channels=16, out_channels=2, kernel_size=7, stride=1, padding=3))
...
class EnhanceBlock(nn.Module):
    def __init__(self,
        ...
        self.pa_deform = DCNv2PackFlowGuided(dim, dim, 3, padding=1, deformable_groups=deformable_groups,
                                                 max_residue_magnitude=max_residue_magnitude, pa_frames=pa_frames, index=index)
