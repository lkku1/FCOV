...
class DeformConv2d_sphe(nn.Module):
    ...
    def return_offset_sphe(self, x, isactiv=False, pad0=False, offset_file=''):
        ...
        # path = './OFFSETS/offset_'+str(w2)+'_'+str(h2)+'_'+str(self.kernel_size[0])+'_'+str(
        #                 self.kernel_size[1])+'_'+str(self.stride[0])+'_'+str(self.stride[1])+'_'+str(self.dilation[0])+'.pt'
        # if not os.path.exists(path):
        #     python_path = sys.executable
        #     conda_prefix = os.path.dirname(python_path)
        #     conda_default_env = os.path.basename(conda_prefix)
        #     os.system(f'call E:\\anaconda3\\Scripts\\activate.bat {conda_default_env} && python create_offset_tensor.py --w {x.shape[-1]}  --h {x.shape[-2]} --k {self.kernel_size[0]} --s {self.stride[0]} --p {self.padding[0]} --d {self.dilation[0]}')
        ...
        if isactiv:
            if offset_file == '':
                if pad0:
                    ...
                    offset_file = './OFFSETS/offset_'+str(w2)+'_'+str(h2)+'_'+str(self.kernel_size[0])+'_'+str(
                        self.kernel_size[1])+'_'+str(self.stride[0])+'_'+str(self.stride[1])+'_'+str(self.dilation[0])+'.pt'
                offset = torch.load(offset_file, weights_only=True).cuda()
    ...
    def forward(self, input: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        if self.init == 0:
            ...
            # self.offset_sphe = self.offset_sphe.type(torch.float32)
            self.init = 1
