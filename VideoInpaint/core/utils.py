...
class GroupRandomHorizontalFlip(object):
    ...
    def __call__(self, img_group, dep_group):
        v = random.random()
        if v < 0.5:
            images = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in img_group]
            depths = [dep.transpose(Image.FLIP_LEFT_RIGHT) for dep in dep_group]

            return images, depths
        else:
            return img_group, dep_group
        
class GroupRandomHorizontalROll(object):
    """Randomly horizontally flips the given PIL.Image with a probability of 0.5
    """

    def __call__(self, img_group, dep_group):
        v = random.random()
        _, _, h, w = img_group.shape
        roll_idx = random.randint(0, w)

        if v < 0.5:
            imgs = torch.roll(img_group, roll_idx, dims=-1)
            deps = torch.roll(dep_group, roll_idx, dims=-1)
            return imgs, deps
        else:
            return img_group, dep_group
